...
from sqlalchemy_filters.exceptions import BadFilterFormat, BadSpec, FieldNotFound
...
SET_NOT_SUPPORTED = "SET type and operators supported only by MySQL"
...
class TestApplyFindInSetFilter:
    ...
    def test_field_in_value_list(
        self, session, is_mysql, value, result_count, result_ids
    ):
        if not is_mysql:
...
class TestTimeFields:
    ...
    @pytest.mark.parametrize(
        "value",
        [datetime.time(3, 5, 9), datetime.time(3, 5, 9).isoformat()],  # '03:05:09'
    )
    @pytest.mark.usefixtures("multiple_quxs_inserted")
    ...
    @pytest.mark.parametrize(
        "value",
        [datetime.time(2, 5, 9), datetime.time(2, 5, 9).isoformat()],  # '02:05:09'
    )
    @pytest.mark.usefixtures("multiple_quxs_inserted")
...
class TestHybridAttributes:
    ...
    def test_orm_descriptors_not_valid_hybrid_attributes(
        ...
        query = session.query(Bar)
        filters = [{"field": field, "op": "==", "value": 100}]
    ...
    def test_filter_by_hybrid_properties(self, session):
        query = session.query(Bar)
        filters = [{"field": "count_square", "op": "==", "value": 100}]
